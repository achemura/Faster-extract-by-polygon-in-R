...
# read the shapefile
dist.shape <- readOGR("~/DATABANK/Ethiopia/Coffee/Specialty/Special_all_pnrj.shp")# My polygon shapefile i want to extract with
for (i in folders){
  ...
  names_1 <- list.files(paste0("~/DATABANK/Ethiopia/Coffee/Model_out/",i,"/Class/"), pattern = ".tif$")
  ...
  # Wriring all the csv files 
  write.csv(df4,paste0("~/DATABANK/Ethiopia/Coffee/Model_out/",i,".csv"),row.names = F)
